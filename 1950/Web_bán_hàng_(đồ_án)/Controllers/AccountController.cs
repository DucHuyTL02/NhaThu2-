    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (acc.Customer.Any(c => c.CustomerEmail== model.Email))
                {
                ...
                if (acc.User.Any(u => u.Username == model.UserName))
                {
        ...
        {
            var user = acc.User.SingleOrDefault(u => u.Username == username && u.Password == password);
            if (user != null)
            {
